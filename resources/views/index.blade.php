...
<body class="bg-neutral-50 min-h-screen flex flex-col text-neutral-950">
    ...
    <!-- Top Navigation Bar -->
    <nav class="shadow-xs w-full top-0 z-50 px-4 md:px-12 bg-white border-b border-neutral-200">
        <div class="flex justify-between items-center py-2 mx-auto max-w-7xl">
            <div class="flex items-center gap-3">
                <img src="/build/assets/logo.png" alt="School Logo" class="p-2 bg-neutral-100/70 border rounded-full border-emerald-100 w-12 h-12">
                <div>
                    <h2 class="text-xl md:text-2xl font-bold text-emerald-700 tracking-tight">QR Attendance</h2>
                    <p class="text-xs text-neutral-500 font-medium">Official Student Attendance Registration</p>
                </div>
            </div>
            <div class="flex items-center space-x-4">
                <a href="{{ route('login') }}" class="btn-filled text-sm px-4 py-2">Log in</a>
            </div>
        </div>
    </nav>

    <!-- Main Content -->
    <main class="flex flex-1 flex-col md:flex-row items-stretch justify-center bg-gradient-to-l from-emerald-50 to-white px-2 md:px-12">

        <section class="flex-1 flex flex-col items-center justify-start py-8 md:py-12 gap-y-6 w-full">
            <!-- Welcome/Info Section -->
            <div class="space-y-2 w-full max-w-md md:max-w-none md:w-[48ch]">
                <h1 class="text-3xl md:text-6xl font-extrabold text-emerald-800 leading-tight">Welcome to QR Code Student Attendance</h1>
                <p class="text-neutral-600 text-base md:text-lg leading-normal">Track and manage student attendance efficiently and securely using QR codes.</p>
            </div>

            <div class="text-neutral-700 text-xs md:text-sm leading-relaxed w-full max-w-md md:max-w-none md:w-[48ch] pl-2 md:pl-4">
        ...
        <section class="flex-1 flex items-center justify-center py-8 md:py-12 w-full">
            <!-- Registration Form Section -->
            <section class="w-full max-w-md md:max-w-lg bg-white rounded-2xl shadow-lg p-4 md:p-8 ring-1 ring-emerald-100">
                <h2 class="text-xl md:text-2xl font-bold text-center mb-4 md:mb-6 text-emerald-800">Student Registration</h2>
                <form method="post" action="" class="space-y-4 md:space-y-5" id="student-form">
                    @csrf
                    @method('post')
    ...
    <footer class="flex">
        <div class="flex items-center gap-2 py-4 mx-auto">
            <img src="/build/assets/school-seal.png" alt="School Seal" class="h-8 w-8 md:h-10 md:w-10 rounded-full border border-neutral-200 bg-white shadow" onerror="this.style.display='none'">
